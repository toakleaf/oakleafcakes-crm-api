...
module.exports = (id, token, name = 'there') => {
  ...
  const paragraph1 = `Thanks for signing up for an account at ${COMPANY_NAME}! Please use the link below to verify your account's email address.  If you believe you have received this email in error please disregard.`;
  const button_text = `Click here to verify account`;
  // this url should point to front end, but include login id and token params. reminder that login id != account id
  const button_url = `https://${COMPANY_SITE}/account/verify/${id}/${token}`;
  const paragraph2 = `Alternatively, you may copy the following link into your browser's URL line: ${button_url}`;
  const salutation = `Sincerely,`;
  const paragraph3 = `The ${COMPANY_NAME} Team`;
  const unsubscribe_url = ``;
  const COMPANY_ADDRESS = `123 Test ST, Test, NY 012345`;
  const preheader = `Action required to begin using your account.`;
